export default function convertToUnicode(textToConvert) {
    const symbolsToReplace = [
        ...
        "f‘", "\"", "'", "+", "f", "[", "\\", "]", "}", "F", "L", "M",
        "्ा", "्ो", "्ौ", "अो", "अा", "आै", "आे", "ाो", "ाॅ", "ाो",
        "ंु", "ेे", "अै", "ाो", "अे", "ंा", "अॅ", "ाै", "ैा", "ंृ",
        "ँा", "ँू", "ेा", "ंे"
    ];
    ...
    return replaceSymbols(textToConvert, symbolCount, symbolsToReplace, correspondingUnicodeSymbols);
}

function replaceSymbols(textToConvert, symbolCount, symbolsToReplace, correspondingUnicodeSymbols) {
    if (textToConvert !== "") {
        for (let symbolIdx = 0; symbolIdx < symbolCount; symbolIdx++) {
            let index = textToConvert.indexOf(symbolsToReplace[symbolIdx]);

            while (index !== -1) {
                textToConvert = textToConvert.replace(symbolsToReplace[symbolIdx], correspondingUnicodeSymbols[symbolIdx]);
        ...
        while (positionOfWrongEe !== -1) {
            ...
            textToConvert = textToConvert.replace(charToReplace, "्" + consonantNextToWrongEe + "ि");
        ...
        while (positionOfReph > 0) {
            ...
            let charAtProbablePositionOfHalfR = textToConvert.charAt(probablePositionOfHalfR);
            while (setOfMatras.includes(charAtProbablePositionOfHalfR)) {
                probablePositionOfHalfR = probablePositionOfHalfR - 1;
                charAtProbablePositionOfHalfR = textToConvert.charAt(probablePositionOfHalfR);
            }
            let charToReplace = textToConvert.substr(probablePositionOfHalfR, (positionOfReph - probablePositionOfHalfR));
            let newReplacementString = "र्" + charToReplace;
            charToReplace = charToConvert + "{";
            textToConvert = textToConvert.replace(charToReplace, newReplacementString);
