const preetiToUnicode = (textToConvert) => {
    ...
    while (hasNextChunk) {
        startIndex = endIndex;
        if (endIndex < textToConvert.length - maxTextSize) {
            endIndex += maxTextSize;
            while (textToConvert.charAt(endIndex) !== ' ') {
                endIndex--;
            }
        } else {
            endIndex = textToConvert.length;
            hasNextChunk = false;
        }
        const chunk = textToConvert.substring(startIndex, endIndex);
...
const replaceSymbols = (chunk, symbolsLength, symbolsOne, symbolsTwo) => {
